...
for i=1:1:NumDays(1)
   ...
   if (~strcmp(next_day,current_day)) % it'a new day...
       mondayz = mondayz +1;
       timesCounter = timesCounter+1;
       times(timesCounter) = str2num(parsed(i,30:31));
   end    
...
timesSize = size(times);
arrival = ones([ timesSize(2), 24]);


for i=1:1:timesSize(2)
   for j = 1:1:24
       if (j>=times(i))
          arrival(i,j) = 2;
       end
...
B = mnrfit( (1:1:24)', minArrival );
pred = mnrval( B, (1:1:24)' );


% notify the front end software
fprintf( 1, 'There is a %0.2f%% chance for the selected user to be at his lab right now.\n', ...
     pred(select,2)*100 );
